...
void CustomFunctionInjector::ThrowEvent(FunctionHandle handle, OsiArgumentDesc * args) const
{
	...
	auto const& globals = gExtender->GetServer().Osiris().GetDynamicGlobals();
	if (globals.OsirisObject == nullptr) {
		OsiError("Attempted to throw event while Osiris is not initialized!");
		return;
	}

	gExtender->GetServer().Osiris().GetWrappers().Event.CallOriginal(globals.OsirisObject, it->second, args);
}
