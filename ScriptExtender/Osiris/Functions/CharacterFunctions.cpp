...
namespace dse::esv
{
	...
	namespace func
	{
		stats::CharacterDynamicStat * GetCharacterDynamicStat(esv::Character * character, uint32_t index)
		{
		...
		void CharacterIterateSkills(OsiArgumentDesc const & args)
		{
			...
			for (auto const& skill : skillEvents) {
				...
				gExtender->GetServer().Osiris().GetCustomFunctionInjector().ThrowEvent(SkillIteratorEventHandle, eventArgs);
