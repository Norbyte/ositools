...
P(CustomDisplayName)
// P(StoryDisplayName)
P_REF(OriginalTransformDisplayName)
P(SpeedMultiplier)
...
BEGIN_CLS(ecl::CharacterBody)
P_RO(CharacterHandle)
P(NextAnimation)
P(CurrentAnimation)
P_REF(AnimationOverride)
P(AnimRandom)
P(ClimbLoopDuration)
P(QueuedAnimation)
P(AnimationRunning)
...
P(WaitForLoad)
P(AnimationDuration)
P(SpeedScaledAnimationDuration)
// TODO - P_REF(ActionState)
END_CLS()
...
P_RO(CorpseOwnerHandle)
P_RO(CorpseCharacterHandle)
P(LootedByHandle)
P(HighlightCircleEffect)
P(ViewConeEffectHandle)
P(FeetStatusId)
P(FootStepEffectTimer)
P(RemainingMoveDistance)
P(FootStepMaterialType)
P_REF(PlayerData)
P_REF(PlayerUpgrade)
P_REF(SurfacePathInfluences)
P_REF(Tags)
P(LifeTime)
P_REF(SoundObjectHandles)
P(FollowCharacterHandle)
P_BITMASK(Flags3)
P_BITMASK(Flags2)
P(Color)
P(Team)
P_REF(DisplayNameOverride)
P_REF(StoryDisplayName)
P_REF(OriginalDisplayName)
P(TalkingIconEffect)
P(AnimationSpeed)
P_RO(SoundBoneIndex)
P_RO(PhysicsBoneIndex)
P_REF(VoiceSet)
P(AnimationSetOverride)
P(WalkSpeedOverride)
P(RunSpeedOverride)
...
P(HealCounter)
P(KillCounter)
P(MovingProjectileHandle)
P(Archetype)
P(EquipmentColor)
P(DialogId)
P(IsDialogAiControlled)
P_REF(SummonHandles)
P_REF(AttitudeMaps)
P_REF(ItemTags)
P(CorpseLootable)
...
P_REF(LoadingVisual)
P(EquipmentSlotMask)
P_BITMASK(EquipmentSlotMask)
P(VisualSetSlotMask)
P_BITMASK(VisualSetSlotMask)
END_CLS()
